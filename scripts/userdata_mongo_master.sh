...
sleep 60
SLAVE_IP=`aws ec2 describe-instances --region $REGION --filters 'Name=instance-state-name,Values=running' 'Name=tag:Name,Values=mongo-slave*' --query 'Reservations[0].Instances[0].PrivateIpAddress'`
